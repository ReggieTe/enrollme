...
{
 return $this->getDetails()[0]['description'];   
}
...
{
     $this->client->update('userschool', $data, "id='$this->_userid' ");
}
...
public function login() {
    $count = 0;
    $result = $this->client->select("select * from userschool WHERE email=:email AND password=:password AND state=1 LIMIT 1", array(":email" => $this->_email, ":password" => $this->_password));
...
public function recordLoginTime($id) {
    ...
    return $this->client->update('userschool', array("lastlogin" => date("d-m-y h:m:s")), "id='$id'");
    
}
public function recordLogOutTime($userid) {
    return $this->client->update('userschool', array("lastlogout" => date("d-m-y h:m:s")), "userid='$userid'");
}
...
public function updatePassword($id,$psswrd) {
    ...
    return $this->client->update('userschool', $data, "id='$id' ");
...
{
    $count=0;
     $result = $this->client->select("select * from userschool");
    foreach ($result as $key => $value) {
           if ($link == Hash::create('sha256', $value['email'],$value['id'])) {
         
               $this->client->delete("userschool","id='".$value['id']." ' AND email ='".$value['email']."'");
             $count++;
           }
...
{
    $count=0;
     $result = $this->client->select("select * from userschool WHERE state=1");
    foreach ($result as $key => $value) {
          $count++;        
        }
        return $count;
...
public function activateAccount() {
    ...
    return $this->client->update('userschool', array("state" => "1"), "email='$this->_email'");
}

private function getDetails() {
    
    return $this->client->select("SELECT * FROM userschool WHERE email=:email LIMIT 1", array(":email" =>  $this->_email));
}
