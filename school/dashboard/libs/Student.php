...
class Student {
    ...
    public function setStudentIdFromSession() {
        ...
        //$this->setUserId($this->_studentid);
    }
    ...
    public function getParentId() {
        ...
        return $this->getStudentDetails()[0]['parent_id'];
    }
    ...
    public function saveChange($data = array()) {
        ...
        return $this->_client->update('students', $data, "schools='$this->_userid' AND id='$this->_studentid' ");
    }

    public function getAllStudents() {

        return $this->_client->select("SELECT * FROM students WHERE schools=:id ", array(":id" => $this->_userid));
    }

    public function delete() {
        if (Upload::deleteFiles($this->_userid, $this->_studentid, 'students')) {
            return $this->_client->delete('students', "id='$this->_studentid' AND schools='" . $this->_userid . "'");
        } else {
            return false;
        }
    }
    ...
    private function getStudentDetails() {
        ...
        return $this->_client->select("SELECT * FROM students WHERE schools=:id AND id=:studentid LIMIT 1", array(":id" => $this->_userid, ':studentid' => $this->_studentid));
    }
